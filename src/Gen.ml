    ...
    *********************************************************)
    ...
    let rec sim(size:int)(nbLeft:int)(next_rules: string queue) map (sizemax:int) leafs = 
      if nbLeft == 0 then
        size
      else  
        if size<sizemax then
          ...
          sim (size+arity)(nbLeft+arity-1)(next_rules) map sizemax leafs 
        else
          size


    
    (*Always start with first rule then if size < sizemax then if there are no next rules then which means i have all the leafs
    that i found then i return the rules else use folder in the fonction get the next rule.*)
      ...
      let rec gen epsilon1 epsilon2 zmin zmax nb_refine zstart =
        ...
        let rec try_gen (nb_try:int) (nb_smaller:int) (nb_bigger:int) : ((tree * int) option * int * int) =
          ...
          begin    
            ...
            let (first_rule,_) = List.hd g in
            Queue.push first_rule queue;
            let res = sim 1 1 queue  map sizemax leafs in
              if res >= sizemin && res <= sizemax then
                begin 
                Random.init rnd;
            	  let result = gen_tree g with_prefix idprefix sizemax y in
                (match result with
                    |(Some tree,size) ->
